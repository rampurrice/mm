// A list of 2048 words, commonly used for generating mnemonic phrases.
// Source: BIP-39 English wordlist
import { ReleaseOrder, LiftingRecord, CmrDepositOrder, DailyStockLog, RiceDeliveryRecord, FrkRecord } from './types';
...
const WORDLIST = [
  ...
  'arrest', 'arrive', 'arrow', 'art', 'artefact', 'artist', 'artwork', 'ask', 'aspect', 'assault', 'asset', 'assign',
  ...
  'bitter', 'black', 'blade', 'blame', 'blanket', 'blast', 'bleed', 'blend', 'bless', 'blind', 'block', 'blog',
  ...
  'category', 'cattle', 'caught', 'cause', 'caution', 'cave', 'ceiling', 'celery', 'cement', 'center', 'century',
  'ceramic', 'certain', 'chair', 'chalk', 'champion', 'change', 'chaos', 'chapter', 'charge', 'chase', 'chat',
  ...
  'defense', 'define', 'defy', 'degree', 'delay', 'deliver', 'demand', 'demise', 'denial', 'dentist', 'deny',
  ...
  'entry', 'envelope', 'episode', 'equal', 'equip', 'era', 'erase', 'erode', 'error', 'erupt', 'escape', 'essay',
  ...
  'grid', 'grief', 'grit', 'grocery', 'group', 'grow', 'grunt', 'guard', 'guess', 'guide', 'guilt', 'guitar',
  'gun', 'gym', 'habit', 'hair', 'half', 'hammer', 'hamster', 'hand', 'happy', 'harbor', 'hard', 'harsh',
  ...
  'leave', 'lecture', 'left', 'leg', 'legal', 'legend', 'leisure', 'lemon', 'lend', 'length', 'lens', 'leopard',
  'lesson', 'letter', 'level', 'liar', 'liberty', 'library', 'license', 'life', 'lift', 'light', 'like', 'limb',
  'limit', 'link', 'lion', 'liquid', 'list', 'little', 'live', 'lizard', 'load', 'loan', 'lobster', 'local',
  ...
  'mom', 'moment', 'monitor', 'monkey', 'monster', 'month', 'moon', 'morning', 'mosquito', 'mother', 'motion',
  'motor', 'mountain', 'mouse', 'move', 'movie', 'much', 'muffin', 'mule', 'multiply', 'muscle', 'museum',
  'mushroom', 'music', 'must', 'mutual', 'myself', 'mystery', 'myth', 'naïve', 'name', 'napkin', 'narrow',
  ...
  'please', 'pledge', 'plenty', 'plug', 'plunge', 'poem', 'poet', 'point', 'polar', 'pole', 'police',
  ...
  'punch', 'pupil', 'puppy', 'purchase', 'purity', 'purpose', 'purse', 'push', 'put', 'puzzle', 'pyramid',
  ...
  'saddle', 'sadness', 'safe', 'sail', 'salad', 'salmon', 'salon', 'salt', 'same', 'sample', 'sand',
  ...
  'skate', 'sketch', 'ski', 'skill', 'skin', 'skirt', 'skull', 'slab', 'slam', 'sleep', 'slender',
  'slice', 'slide', 'slight', 'slim', 'slogan', 'slot', 'slow', 'slush', 'small', 'smart', 'smile',
  'smoke', 'smooth', 'snack', 'snake', 'snap', 'sniff', 'snow', 'soap', 'soccer', 'social', 'sock',
  'soda', 'soft', 'solar', 'soldier', 'solid', 'solution', 'solve', 'someone', 'song', 'soon', 'sorry',
  'sort', 'soul', 'sound', 'soup', 'source', 'south', 'space', 'spare', 'spark', 'speak', 'special',
  ...
  'timber', 'time', 'tiny', 'tip', 'tired', 'tissue', 'title', 'toast', 'tobacco', 'today', 'toe',
  ...
  'travel', 'tray', 'treat', 'tree', 'trend', 'trial', 'tribe', 'trick', 'trigger', 'trim', 'trip',
 ...
 * @returns A string containing 12 space-separated words.
 */
export const generateRecoveryPhrase = (): string => {
    const phraseWords = [];
    for (let i = 0; i < 12; i++) {
        const randomIndex = Math.floor(Math.random() * WORDLIST.length);
        phraseWords.push(WORDLIST[randomIndex]);
    }
...
/**
 * Safely loads and parses an array of a specific type from localStorage.
 * @param key The localStorage key.
 * @param validator A function to check if an object conforms to the expected type.
 * @returns An array of the specified type, or an empty array if data is missing, malformed, or invalid.
 */
export const getSafely = <T>(key: string, validator: (item: any) => item is T): T[] => {
    ...
    try {
        ...
        console.error(`Error parsing or validating data from localStorage key "${key}":`, error);
        return [];
    }
};

/**
 * Safely loads data from localStorage and returns it as an array if it's array-like.
 * Used for data that needs migration before strict type validation.
 * @param key The localStorage key.
 * @returns The parsed array, or an empty array if data is missing or not an array.
 */
export const getRawArraySafely = (key: string): any[] => {
    const data = localStorage.getItem(key);
    if (!data) return [];
    try {
        const parsed = JSON.parse(data);
...
export const isReleaseOrder = (item: any): item is ReleaseOrder => {
    return item && typeof item === 'object' && typeof item.doNo === 'string' && typeof item.quantity === 'string';
};

export const isLiftingRecord = (item: any): item is LiftingRecord => {
    return item && typeof item === 'object' && typeof item.id === 'string' && typeof item.netPaddyQuantity === 'number';
};

export const isCmrDepositOrder = (item: any): item is CmrDepositOrder => {
    return item && typeof item === 'object' && typeof item.id === 'string' && typeof item.orderNo === 'string';
};

export const isDailyStockLog = (item: any): item is DailyStockLog => {
    return item && typeof item === 'object' && typeof item.id === 'string' && typeof item.date === 'string';
};

export const isRiceDeliveryRecord = (item: any): item is RiceDeliveryRecord => {
    return item && typeof item === 'object' && typeof item.id === 'string' && typeof item.quantityDeliveredQtls === 'number';
};

export const isFrkRecord = (item: any): item is FrkRecord => {
    return item && typeof item === 'object' && typeof item.id === 'string' && typeof item.quantityQtls === 'number';
};
